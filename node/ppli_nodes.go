...
func (n *PPLICorrelateNode) Execute(ctx NodeContext) (NodeOutput, error) {
	...
	switch {
	case len(msg.TrackID) >= 3 && msg.TrackID[:3] == "NEW":
		result = model.CorrelationNew
	case len(msg.TrackID) >= 3 && msg.TrackID[:3] == "UPD":
		result = model.CorrelationUpdate
	case len(msg.TrackID) >= 3 && msg.TrackID[:3] == "DUP":
...
func (n *StorePPLINode) Execute(ctx NodeContext) (NodeOutput, error) {
	...
	if err := n.db.Create(msg.TrackID, msg); err != nil {
		if updateErr := n.db.Update(msg.TrackID, msg); updateErr != nil {
			return nil, fmt.Errorf("failed to store track: %w", updateErr)
		}
		return NodeOutput{"message": msg, "store_result": "updated"}, nil
	}
